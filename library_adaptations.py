...
def make_classification(
    ...
    centroids *= 2 * intra_class_sep
    centroids -= intra_class_sep

    for i in range(n_classes):
        centroids[i * n_clusters_per_class: (i + 1) * n_clusters_per_class] += i * class_sep
    ...
    for k, centroid in enumerate(centroids):
        ...
        class_label = k // n_clusters_per_class
        client_idx = k % n_clusters_per_class
        y[start:stop] = class_label
        z[start:stop] = client_idx
    ...
    if shuffle:
        ...
        X, y, z = util_shuffle(X, y, z, random_state=generator)
